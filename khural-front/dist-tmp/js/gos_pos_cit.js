(function() {
  ...
  if (!document.querySelector('script[src="https://pos.gosuslugi.ru/bin/script.min.js"]')) {
    const externalScript = document.createElement('script');
    externalScript.src = 'https://pos.gosuslugi.ru/bin/script.min.js';
    document.head.appendChild(externalScript);
  }

  // Добавляем стили
  if (!document.getElementById('gos-pos-cit-styles')) {
    const style1 = document.createElement('style');
    style1.id = 'gos-pos-cit-styles';
    style1.textContent = `
  ...
  // Если document.currentScript не работает (динамическая загрузка), ищем по ID
  if (!container) {
    var script = document.querySelector('script[src*="gos_pos_cit.js"]');
    if (script) container = script.parentNode;
  }
  // Если все еще не нашли, используем последний загруженный скрипт
  if (!container) {
    var scripts = document.getElementsByTagName('script');
    var lastScript = scripts[scripts.length - 1];
    if (lastScript) container = lastScript.parentNode;
  }
  if (!container) return;

  // Проверяем, не создан ли уже виджет в этом контейнере
  if (container.querySelector('#js-show-iframe-wrapper')) {
    return; // Виджет уже создан, выходим
  }

  // Создаем HTML структуру виджета
  var wrapper = document.createElement('div');
  wrapper.id = 'js-show-iframe-wrapper';
  wrapper.innerHTML = `
  ...
  (function(widgetElement){
    ...
    var POS_PREFIX_2="--pos-banner-fluid-2__",posOptionsInitial={"grid-template-columns":"100%","grid-template-rows":"310px auto","decor-grid-column":"initial","decor-grid-row":"initial","decor-padding":"30px 30px 0 30px","bg-url":"url('https://pos.gosuslugi.ru/bin/banner-fluid/2/banner-fluid-bg-2-small.svg')","bg-position":"calc(10% + 64px) calc(100% - 20px)","bg-size":"cover","content-padding":"0 30px 30px 30px","slogan-font-size":"20px","slogan-line-height":"32px","logo-wrap-padding":"20px 30px 30px 40px","logo-wrap-top":"0","logo-wrap-bottom":"initial","logo-wrap-border-radius":"0 0 0 80px"},setStyles=function(e,t){Object.keys(e).forEach(function(o){t.style.setProperty(POS_PREFIX_2+o,e[o])})},removeStyles=function(e,t){Object.keys(e).forEach(function(e){t.style.removeProperty(POS_PREFIX_2+e)})};
    function changePosBannerOnResize(){var e=document.documentElement,t=_objectSpread({},posOptionsInitial),o=widgetElement||document.getElementById("js-show-iframe-wrapper"),n=o?o.offsetWidth:document.body.offsetWidth;if(n>405)t["slogan-font-size"]="24px",t["logo-wrap-padding"]="30px 50px 30px 70px";if(n>500)t["grid-template-columns"]="min-content 1fr",t["grid-template-rows"]="100%",t["decor-grid-column"]="2",t["decor-grid-row"]="1",t["decor-padding"]="30px 30px 30px 0",t["content-padding"]="30px",t["bg-position"]="0% calc(100% - 70px)",t["logo-wrap-padding"]="30px 30px 24px 40px",t["logo-wrap-top"]="initial",t["logo-wrap-bottom"]="0",t["logo-wrap-border-radius"]="80px 0 0 0";if(n>585)t["bg-position"]="0% calc(100% - 6px)";if(n>800)t["bg-url"]="url('https://pos.gosuslugi.ru/bin/banner-fluid/2/banner-fluid-bg-2.svg')",t["bg-position"]="0% center";if(n>1020)t["slogan-font-size"]="32px",t["line-height"]="40px",t["logo-wrap-padding"]="30px 30px 24px 50px";setStyles(t,e)}
  ...
  // Инициализация виджета после загрузки внешнего скрипта
  function initWidget() {
  ...
  // Ждем загрузки внешнего скрипта
  const externalScript = document.querySelector('script[src="https://pos.gosuslugi.ru/bin/script.min.js"]');
  if (externalScript && externalScript.complete) {
    initWidget();
  } else if (externalScript) {
    externalScript.onload = initWidget;
  } else {
