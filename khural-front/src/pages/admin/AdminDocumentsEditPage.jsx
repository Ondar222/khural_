...
import AdminDocumentsEdit from "./AdminDocumentsEdit.jsx";
import { useAdminData } from "../../hooks/useAdminData.js";

export default function AdminDocumentsEditPage() {
  ...
  const documentId = React.useMemo(() => {
    ...
    const match = base.match(/\/admin\/documents\/(\d+)$/);
    if (match) {
  ...
  if (!documentId) {
    return (
      ...
      >
        <div className="admin-card">Неверный ID документа</div>
      </AdminShell>
    );
  }
  ...
  return (
    ...
    >
      {loginCard}
      <AdminDocumentsEdit documentId={documentId} onUpdate={adminData.updateDocument} busy={adminData.busy} canWrite={adminData.canWrite} />
    </AdminShell>
  );
}
